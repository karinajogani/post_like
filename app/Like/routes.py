...
def like_post(like : LikePy, db: Session = Depends(get_db)):
    ...
    post_user_id_filed = (db.query(Post.created_by).filter(Post.id == new_like.post_id).first())
    ...
    if public_or_private == "public": return post_like_function(db, new_like)
        ...
        if new_like.created_by == post_user_id: return post_like_function(db, new_like)
        ...
        if new_like.created_by in display_user_list or new_like.created_by == post_user_id:
...
def post_details(post_id: str, created_by: str, db: Session = Depends(get_db)):

    post_user_id_column = db.query(Post.created_by).filter(Post.id == post_id).first()
    post_user_id = post_user_id_column["created_by"]
    if post_user_id == created_by:
        likes_user_id_filed = (db.query(Like.created_by).filter(Like.post_id == post_id).all())

        like_user_id_list = []
        for i in range(len(likes_user_id_filed)):
            likes_user_id = likes_user_id_filed[i]["created_by"]
